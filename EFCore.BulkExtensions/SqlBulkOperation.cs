...
namespace EFCore.BulkExtensions
{
    ...
    {
        internal static string ColumnMappingExceptionMessage =>
            ...
            TableInfo tableInfo)
        {
            ...
            var entityPropertiesDict = entityType.GetProperties()
                .Where(a => tableInfo.PropertyColumnNamesDict.ContainsKey(a.Name)).ToDictionary(a => a.Name, a => a);
            var entityNavigationOwnedDict = entityType.GetNavigations().Where(a => a.GetTargetType().IsOwned())
                .ToDictionary(a => a.Name, a => a);
            var properties = type.GetProperties();
            var discriminatorColumn =
                tableInfo.ShadowProperties.Count == 0 ? null : tableInfo.ShadowProperties.ElementAt(0);
            ...
            foreach (var property in properties)
                ...
                {
                    var navOwnedType = type.Assembly.GetType(property.PropertyType.FullName);

                    var ownedEntityType = context.Model.FindEntityType(property.PropertyType);
                    if (ownedEntityType == null)
                        ownedEntityType = context.Model.GetEntityTypes().SingleOrDefault(a =>
                            a.DefiningNavigationName == property.Name && a.DefiningEntityType.Name == entityType.Name);
                    ...
                    foreach (var ownedEntityProperty in ownedEntityProperties)
                        ...
                        {
                            var columnName = ownedEntityProperty.Relational().ColumnName;
            ...
            foreach (var entity in entities)
            {
                foreach (var property in properties)
                {
                    ...
                    if (entityPropertiesDict.ContainsKey(property.Name))
                    {
                        var columnName = entityPropertiesDict[property.Name].Relational().ColumnName;
                        if (tableInfo.ConvertibleProperties.ContainsKey(columnName))
                            propertyValue = tableInfo.ConvertibleProperties[columnName].ConvertToProvider
                    ...
                    {
                        var ownedProperties = property.PropertyType.GetProperties().Where(a =>
                            ownedEntitiesMappedProperties.Contains(property.Name + "_" + a.Name));
                        foreach (var ownedProperty in ownedProperties)
                            columnsDict[property.Name + "_" + ownedProperty.Name] = propertyValue == null
                                ? null
                                : ownedProperty.GetValue(propertyValue, null);
                    }
                }
        ...
        public static void Insert<T>(DbContext context, IList<T> entities, TableInfo tableInfo,
            Action<decimal> progress)
        {
            ...
            try
            {
                ...
                {
                    var useFastMember =
                        tableInfo.HasOwnedTypes ==
                        false // With OwnedTypes DataTable is used since library FastMember can not (https://github.com/mgravell/fast-member/issues/21)
                        && tableInfo.ColumnNameContainsSquareBracket ==
                        false // FastMember does not support escaped columnNames  ] -> ]]
                        &&
                        tableInfo.ShadowProperties.Count ==
                        0 // With Shadow prop. Discriminator (TPH inheritance) also not used because FastMember is slow for Update (https://github.com/borisdj/EFCore.BulkExtensions/pull/17)
                        && !tableInfo.ConvertibleProperties
                            .Any() // With ConvertibleProperties FastMember is slow as well
                        && !tableInfo.HasAbstractList // AbstractList not working with FastMember
                        && !tableInfo.BulkConfig.UseOnlyDataTable;
                    var setColumnMapping = useFastMember;
                    ...
                    try
                    {
                        if (useFastMember)
                        {
                            using (var reader = ObjectReaderEx.Create(entities, tableInfo.ShadowProperties,
                                tableInfo.ConvertibleProperties, context,
                                tableInfo.PropertyColumnNamesDict.Keys.ToArray()))
                            {
                        ...
                        {
                            var dataTable = GetDataTable(context, entities, sqlBulkCopy, tableInfo);
                        ...
                        if (ex.Message.Contains(ColumnMappingExceptionMessage))
                            if (!tableInfo.CheckTableExist(context, tableInfo))
                            {
                                context.Database.ExecuteSqlCommand(
                                    SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                                        tableInfo.FullTempTableName,
                                        tableInfo)); // Will throw Exception specify missing db column: Invalid column name ''
                                context.Database.ExecuteSqlCommand(
                                    SqlQueryBuilder.DropTable(tableInfo.FullTempTableName));
                            }
        ...
        public static async Task InsertAsync<T>(DbContext context, IList<T> entities, TableInfo tableInfo,
            Action<decimal> progress)
        {
            ...
            try
            {
                using (var sqlBulkCopy = GetSqlBulkCopy(sqlConnection, transaction, tableInfo.BulkConfig))
                {
                    var useFastMember = tableInfo.HasOwnedTypes == false
                        ...
                        if (ex.Message.Contains(ColumnMappingExceptionMessage))
                            if (!await tableInfo.CheckTableExistAsync(context, tableInfo))
                            {
                                await context.Database.ExecuteSqlCommandAsync(
                                    SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                                        tableInfo.FullTempTableName, tableInfo));
                                await context.Database.ExecuteSqlCommandAsync(
                                    SqlQueryBuilder.DropTable(tableInfo.FullTempTableName));
                            }
        ...
        public static void Merge<T>(DbContext context, IList<T> entities, TableInfo tableInfo,
            OperationType operationType, Action<decimal> progress) where T : class
        {
            tableInfo.InsertToTempTable = true;
            tableInfo.CheckHasIdentity(context);

            context.Database.ExecuteSqlCommand(SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                tableInfo.FullTempTableName, tableInfo));
            if (tableInfo.CreatedOutputTable)
            {
                context.Database.ExecuteSqlCommand(SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                    tableInfo.FullTempOutputTableName, tableInfo, true));
                if (tableInfo.TimeStampColumnName != null)
                    context.Database.ExecuteSqlCommand(SqlQueryBuilder.AddColumn(tableInfo.FullTempOutputTableName,
                        tableInfo.TimeStampColumnName, tableInfo.TimeStampOutColumnType));
            }

            var keepIdentity = tableInfo.BulkConfig.SqlBulkCopyOptions.HasFlag(SqlBulkCopyOptions.KeepIdentity);
            try
            {
                ...
                if (keepIdentity && tableInfo.HasIdentity)
                {
                    ...
                    context.Database.ExecuteSqlCommand(
                        SqlQueryBuilder.SetIdentityInsert(tableInfo.FullTableName, true));
                }

                context.Database.ExecuteSqlCommand(SqlQueryBuilder.MergeTable(tableInfo, operationType));

                if (tableInfo.CreatedOutputTable) tableInfo.LoadOutputData(context, entities);
            }
            finally
            {
                if (!tableInfo.BulkConfig.UseTempDB)
                {
                    if (tableInfo.CreatedOutputTable)
                        context.Database.ExecuteSqlCommand(
                            SqlQueryBuilder.DropTable(tableInfo.FullTempOutputTableName));
                ...
                {
                    context.Database.ExecuteSqlCommand(
                        SqlQueryBuilder.SetIdentityInsert(tableInfo.FullTableName, false));
                    context.Database.CloseConnection();
                }
            }
        }

        public static async Task MergeAsync<T>(DbContext context, IList<T> entities, TableInfo tableInfo,
            OperationType operationType, Action<decimal> progress) where T : class
        {
            tableInfo.InsertToTempTable = true;
            ...
            await context.Database
                .ExecuteSqlCommandAsync(SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                    tableInfo.FullTempTableName, tableInfo)).ConfigureAwait(false);
            if (tableInfo.CreatedOutputTable)
            {
                await context.Database
                    .ExecuteSqlCommandAsync(SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName,
                        tableInfo.FullTempOutputTableName, tableInfo, true)).ConfigureAwait(false);
                if (tableInfo.TimeStampColumnName != null)
                    context.Database.ExecuteSqlCommand(SqlQueryBuilder.AddColumn(tableInfo.FullTempOutputTableName,
                        tableInfo.TimeStampColumnName, tableInfo.TimeStampOutColumnType));
            }
            ...
            finally
            {
                if (!tableInfo.BulkConfig.UseTempDB)
                {
                    if (tableInfo.CreatedOutputTable)
                        await context.Database
                            .ExecuteSqlCommandAsync(SqlQueryBuilder.DropTable(tableInfo.FullTempOutputTableName))
                            .ConfigureAwait(false);
                    await context.Database
                        .ExecuteSqlCommandAsync(SqlQueryBuilder.DropTable(tableInfo.FullTempTableName))
                        .ConfigureAwait(false);
                }
        ...
        public static void Read<T>(DbContext context, IList<T> entities, TableInfo tableInfo, Action<decimal> progress)
            where T : class
        {
            var previousPropertyColumnNamesDict = tableInfo.ConfigureBulkReadTableInfo(context);
        ...
        public static async Task ReadAsync<T>(DbContext context, IList<T> entities, TableInfo tableInfo,
            Action<decimal> progress) where T : class
        {
            var previousPropertyColumnNamesDict = tableInfo.ConfigureBulkReadTableInfo(context);

            await context.Database.ExecuteSqlCommandAsync(
                SqlQueryBuilder.CreateTableCopy(tableInfo.FullTableName, tableInfo.FullTempTableName, tableInfo));
