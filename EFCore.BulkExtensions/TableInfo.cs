...
namespace EFCore.BulkExtensions
{
    public class TableInfo
    {
        ...
        public Dictionary<string, string> OutputPropertyColumnNamesDict { get; set; } =
        ...
        public Dictionary<string, ValueConverter> ConvertibleProperties { get; set; } =
            new Dictionary<string, ValueConverter>();

        public string TimeStampOutColumnType => "varbinary(8)";
        public string TimeStampColumnName { get; set; }

        public static TableInfo CreateInstance<T>(DbContext context, IList<T> entities, OperationType operationType,
            BulkConfig bulkConfig)
        {
            ...
            if (tableInfo.BulkConfig.UseTempDB && !isExplicitTransaction &&
                (operationType != OperationType.Insert || tableInfo.BulkConfig.SetOutputIdentity))
                throw new InvalidOperationException(
                    "UseTempDB when set then BulkOperation has to be inside Transaction. More info in README of the library in GitHub.");
            ...
            tableInfo.LoadData(context, entities, isDeleteOperation);
            return tableInfo;
        }
        ...
        public static string GetUniquePropertyValues<T>(T entity, List<string> propertiesNames, TypeAccessor accessor)
        {
            ...
            foreach (var propertyName in propertiesNames) result += accessor[entity, propertyName];
            return result;
        }
        ...
        protected void UpdateEntitiesIdentity<T>(IList<T> entities, IList<T> entitiesWithOutputIdentity)
        {
            if (BulkConfig.PreserveInsertOrder) // Updates PK in entityList
            {
                ...
                for (var i = 0; i < NumberOfEntities; i++)
                    UpdateEntityPrimaryKeyProperties(accessor, localEntities[i], localEntitiesWithOutputIdentity[i]);
            }
        ...
        private void UpdateEntityPrimaryKeyProperties<T>(
            TypeAccessor accessor,
            T entity,
            T EntityWithOutputIdentity)
        {
            PrimaryKeys.ForEach(name => { accessor[entity, name] = accessor[EntityWithOutputIdentity, name]; });
        }
        ...
        public void LoadData<T>(DbContext context, IList<T> entities, bool loadOnlyPKColumn)
        {
            ...
            TempTableSufix =
                "Temp" + Guid.NewGuid().ToString()
                    .Substring(0,
                        8); // 8 chars of Guid as tableNameSufix to avoid same name collision with other tables
            ...
            var timestampDbTypeName = nameof(TimestampAttribute).Replace("Attribute", "").ToLower(); // = "timestamp";
            var timeStampProperties = allProperties.Where(a =>
                a.IsConcurrencyToken && a.ValueGenerated == ValueGenerated.OnAddOrUpdate ||
                a.Relational().ColumnType == timestampDbTypeName);
            ...
            OutputPropertyColumnNamesDict = allPropertiesExceptTimeStamp.Concat(timeStampProperties)
                .ToDictionary(a => a.Name,
                    b => b.Relational().ColumnName.Replace("]", "]]")); // square brackets have to be escaped
            ColumnNameContainsSquareBracket = allPropertiesExceptTimeStamp.Concat(timeStampProperties)
                .Any(a => a.Relational().ColumnName.Contains("]"));
            ...
            {
                if (AreSpecifiedUpdateByProperties
            ...
            if (AreSpecifiedPropertiesToInclude || AreSpecifiedPropertiesToExclude)
            {
                if (AreSpecifiedPropertiesToInclude && AreSpecifiedPropertiesToExclude)
                    throw new InvalidOperationException(
                        "Only one group of properties, either PropertiesToInclude or PropertiesToExclude can be specifed, specifying both not allowed.");
                if (AreSpecifiedPropertiesToInclude)
            ...
            if (loadOnlyPKColumn)
            {
                PropertyColumnNamesDict = properties.Where(a => PrimaryKeys.Contains(a.Name))
                    .ToDictionary(a => a.Name, b => b.Relational().ColumnName.Replace("]", "]]"));
            }
            else
            {
                PropertyColumnNamesDict =
                    properties.ToDictionary(a => a.Name, b => b.Relational().ColumnName.Replace("]", "]]"));
                ShadowProperties = new HashSet<string>(properties.Where(p => p.IsShadowProperty)
                ...
                if (HasOwnedTypes) // Support owned entity property update. TODO: Optimize
                    foreach (var navgationProperty in ownedTypes)
                    {
                        ...
                        ) // when entity has more then one ownedType (e.g. Address HomeAddress, Address WorkAddress) or one ownedType is in multiple Entities like Audit is usually.
                            ownedEntityType = context.Model.GetEntityTypes().SingleOrDefault(a =>
                                a.DefiningNavigationName == property.Name &&
                                a.DefiningEntityType.Name == entityType.Name);
                        ...
                        foreach (var ownedProperty in ownedProperties)
                            ...
                            {
                                var columnName = ownedEntityPropertyNameColumnNameDict[ownedProperty.Name];
                                var ownedPropertyType = Nullable.GetUnderlyingType(ownedProperty.PropertyType) ??
        ...
        public void SetSqlBulkCopyConfig<T>(SqlBulkCopy sqlBulkCopy, IList<T> entities, bool setColumnMapping,
            Action<decimal> progress)
        {
            ...
            {
                progress?.Invoke((decimal) (e.RowsCopied * 10000 / entities.Count) /
        ...
        public async Task CheckHasIdentityAsync(DbContext context)
        {
            ...
            {
                if (currentTransaction == null) sqlConnection.Close();
            }
        }
            ...
            try
            {
                ...
                using (var command = sqlConnection.CreateCommand())
                {
                    if (currentTransaction != null)
                        command.Transaction = currentTransaction.GetDbTransaction();
                    command.CommandText =
                        SqlQueryBuilder.CheckTableExist(tableInfo.FullTempTableName, tableInfo.BulkConfig.UseTempDB);
                    using (var reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                            while (reader.Read())
        ...
        protected int GetNumberUpdated(DbContext context)
        {
            ...
            var sqlQueryCount = SqlQueryBuilder.SelectCountIsUpdateFromOutputTable(this);
        ...
        public Dictionary<string, string> ConfigureBulkReadTableInfo(DbContext context)
        {
            ...
            PropertyColumnNamesDict = PropertyColumnNamesDict.Where(a => PrimaryKeys.Contains(a.Key))
        ...
        public void UpdateReadEntities<T>(IList<T> entities, IList<T> existingEntities)
        {
            ...
            if (HasOwnedTypes)
                foreach (var ownedTypeName in OwnedTypesDict.Keys)
                {
        ...
        {
            if (BulkConfig.SetOutputIdentity && HasGenerateValueProperties())
            {
        ...
        private bool HasGenerateValueProperties()
        {
            var valueElements = PrimaryKeyProperties.Select(e => e.ValueGenerated).ToImmutableHashSet();
            return !(valueElements.Count == 1 && valueElements.First() == ValueGenerated.Never);
        }
        ...
        public async Task LoadOutputDataAsync<T>(DbContext context, IList<T> entities) where T : class
        {
            if (BulkConfig.SetOutputIdentity && HasGenerateValueProperties())
            {
                ...
                var entitiesWithOutputIdentity =
                    await QueryOutputTableAsync<T>(context, sqlQuery).ToListAsync().ConfigureAwait(false);
                UpdateEntitiesIdentity(entities, entitiesWithOutputIdentity);
            }
        ...
        private static Expression<Func<DbContext, IQueryable<T>>> OrderBy<T>(
            Expression<Func<DbContext, IQueryable<T>>> source, string ordering)
        {
            ...
            var resultExp = Expression.Call(typeof(Queryable), "OrderBy", new[] {entityType, property.PropertyType},
                source.Body, Expression.Quote(orderByExp));
