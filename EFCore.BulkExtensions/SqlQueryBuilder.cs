...
namespace EFCore.BulkExtensions
{
    public static class SqlQueryBuilder
    {
            ...
            bool isOutputTable = false)
        {
            ...
            var columnsNames =
                (isOutputTable ? tableInfo.OutputPropertyColumnNamesDict : tableInfo.PropertyColumnNamesDict).Values
                .ToList();
            if (tableInfo.TimeStampColumnName != null) columnsNames.Remove(tableInfo.TimeStampColumnName);
            var isUpdateStatsColumn = tableInfo.BulkConfig.CalculateStats && isOutputTable
                ? ",[IsUpdate] = CAST(0 AS bit)"
                : "";
        ...
        {
            var sourceTable = tableInfo.FullTableName;
            var joinTable = tableInfo.FullTempTableName;
            var columnsNames = tableInfo.PropertyColumnNamesDict.Values.ToList();
            var selectByPropertyNames = tableInfo.PropertyColumnNamesDict
                .Where(a => tableInfo.PrimaryKeys.Contains(a.Key)).Select(a => a.Value).ToList();
        ...
        {
            var ON_OFF = identityInsert ? "ON" : "OFF";
        ...
        public static string MergeTable(TableInfo tableInfo, OperationType operationType)
        {
            ...
            var nonIdentityColumnsNames = columnsNames
                .Where(a => !a.Equals(tableInfo.IdentityColumnName, StringComparison.OrdinalIgnoreCase)).ToList();
            var insertColumnsNames = tableInfo.HasIdentity && !keepIdentity ? nonIdentityColumnsNames : columnsNames;

            var isUpdateStatsValue = tableInfo.BulkConfig.CalculateStats
                ? ",(CASE $action WHEN 'UPDATE' THEN 1 Else 0 END)"
                : "";
            ...
            var textWITH_HOLDLOCK = tableInfo.BulkConfig.WithHoldlock ? " WITH (HOLDLOCK)" : "";
            ...
            if (operationType == OperationType.InsertOrUpdateDelete) q += " WHEN NOT MATCHED BY SOURCE THEN DELETE";
            if (operationType == OperationType.Delete) q += " WHEN MATCHED THEN DELETE";
        ...
        public static string GetANDSeparatedColumns(List<string> columnsNames, string prefixTable = null,
            string equalsTable = null, bool updateByPropertiesAreNullable = false)
        {
